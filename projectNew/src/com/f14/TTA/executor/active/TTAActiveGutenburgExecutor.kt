...
import com.f14.TTA.executor.TTATakeCardExecutor
...
class TTAActiveGutenburgExecutor(param: RoundParam, card: TTACard) : TTAActiveExecutor(param, card) {
//    val ability = card.activeAbility ?: throw BoardGameException("能力出错!")

    @Throws(BoardGameException::class)
    override fun active() {
        val listener = TTARequestSelectListener(gameMode, player, card, TTACmdString.REQUEST_SELECT, "拿牌,建造,升级,出牌", "请选择你想要进行的动作")
        val res = gameMode.insertListener(listener)
        when (res.getInteger("sel")) {
            0 -> this.actived = requestTakeCard()
            1 -> this.actived = requestBuild()
            2 -> this.actived = requestUpgrade()
            3 -> this.actived = requestPlayCard()
        }
    }

    private fun requestTakeCard(): Boolean {
        val listener = TTARequestSelectCardListener(gameMode, player, card, TTACmdString.ACTION_TAKE_CARD, "请选择要拿取的牌!")
        listener.condition = this.ability
        val res = gameMode.insertListener(listener)
        val subact = res.getString("subact")
        if (TTACmdString.ACTION_TAKE_CARD == subact) {
            val takeCard = res.get<TTACard>("card") ?: throw BoardGameException("找不到这张牌")
            val index = gameMode.cardBoard.getCardIndex(takeCard.id)
            val executor = TTATakeCardExecutor(param, takeCard.id)
    ...
    private fun requestBuild(): Boolean {
        ...
        if (TTACmdString.ACTION_BUILD == subact) {
            val buildCard = res.get<TTACard>("card") ?: throw BoardGameException("找不到这张牌")
    ...
    private fun requestPlayCard(): Boolean {
        ...
        if (TTACmdString.ACTION_PLAY_CARD == subact) {
            val playCard = res.get<TechCard>("card") ?: throw BoardGameException("找不到这张牌")
    ...
    private fun requestUpgrade(): Boolean {
        ...
        if (TTACmdString.REQUEST_UPGRADE_TO == subact) {
            val fromCard = res.get<TechCard>("card") ?: throw BoardGameException("找不到这张牌")
            ...
            if (TTACmdString.ACTION_UPGRADE == subact) {
                val toCard = res.get<TechCard>("card") ?: throw BoardGameException("找不到这张牌")
