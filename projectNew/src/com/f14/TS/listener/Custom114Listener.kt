 ...
 * @author F14eagle
 */
class Custom114Listener(trigPlayer: TSPlayer, gameMode: TSGameMode, initParam: InitParam) : TSParamInterruptListener(trigPlayer, gameMode, initParam) {
    private var defcon: Int = 0
    ...
    override fun doConfirm(action: GameAction) {
        ...
        if (defcon != 4 && defcon != 5) {
            throw BoardGameException("你只能把DEFCON设为等级4或等级5!")
        }
        this.defcon = defcon
