 ...
 * @author F14eagle
 */
class InnoCustom021Listener(gameMode: InnoGameMode, trigPlayer: InnoPlayer, initParam: InnoInitParam?, resultParam: InnoResultParam, ability: InnoAbility?, abilityGroup: InnoAbilityGroup?) : InnoCommonConfirmListener(gameMode, trigPlayer, initParam, resultParam, ability, abilityGroup) {
    ...
    override fun doConfirm(action: GameAction) {
        ...
        val handEmpty = player.hands.isEmpty
        val scoreEmpty = player.scores.isEmpty
        val handCards = ArrayList(player.hands.maxLevelCards)
        val scoreCards = ArrayList(player.scores.maxLevelCards)
        // 允许空换!!
        if (!handEmpty) {
